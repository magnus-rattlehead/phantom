...
static void test_non_ascii_stripped(void) {
    /* All non-ASCII bytes dropped; ASCII chars preserved. */
    const char *src = "caf\xc3\xa9";
    char dst[64];
    size_t n = strip_ansi(src, strlen(src), dst, sizeof dst);
    assert(3 == n);
    assert(0 == strcmp(dst, "caf"));
}
...
static void test_unicode_stripped(void) {
    /* Box-drawing, braille, bullet all dropped; surrounding ASCII kept. */
    const char *src = "a\xe2\x94\x80"
                      "b\xe2\xa0\x80"
                      "c\xe2\x80\xa2"
                      "d";
    char dst[64];
