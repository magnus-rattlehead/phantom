...
struct SearchState {
    ...
    int     results_dirty;
    int     current;
    int     live_sb_base;  /* sb->total_rows at search trigger time */
    int     event_pushed;  /* 1 after first render event sent this run */
};
...
static void search_result_cb(int abs_row, void *arg)
{
    ...
    if (s->results_count == s->results_cap) {
        ...
        if (!newbuf) { pthread_mutex_unlock(&s->lock); return; }
        s->results     = newbuf;
...
static void trigger_search(SearchState *s, Terminal *t)
{
    ...
    if (s->query_len > 0) {
        SB_LOG("trigger query=\"%s\" live_sb_base=%d",
               s->query, s->live_sb_base);
...
static void navigate(SearchState *s, Terminal *t, int dir)
{
    ...
    pthread_mutex_lock(&s->lock);
    int abs_row = s->results[s->current];
    pthread_mutex_unlock(&s->lock);
    SB_LOG("navigate %s -> idx=%d abs_row=%d",
           dir > 0 ? "next" : "prev", s->current, abs_row);
    terminal_scroll_to_row(t, abs_row);
}

int search_handle_key(SearchState *s, Terminal *t,
                       ...
                       const SDL_TextInputEvent *e)
{
    ...
    if (s->query_len + len + 1 > s->query_cap) {
        ...
        while (new_cap < s->query_len + len + 1) new_cap *= 2;
...
int *search_snapshot(SearchState *s, int *out_count)
{
    ...
    *out_count = s->results_count;
    ...
    if (s->results_count > 0) {
        ...
        if (snap)
            memcpy(snap, s->results,
                   (size_t)s->results_count * sizeof(int));
        else
            *out_count = 0;
    }
    pthread_mutex_unlock(&s->lock);
