...
/* On macOS, TEXT_INPUT is not fired during key-repeat when the system
 * "Press and Hold" accent picker intercepts it; synthesize the char here
 * so held keys still produce output. */
...
void input_handle_event(const SDL_Event *event, Terminal *term, Pty *pty) {
    if (SDL_EVENT_MOUSE_WHEEL == event->type) {
        terminal_scroll(term,
                        (int)(event->wheel.y * (float)SCROLL_LINES_PER_TICK));
        return;
    }
    ...
    if (SDL_EVENT_TEXT_INPUT == event->type) {
        ...
        if (len)
            pty_write(pty, event->text.text, len);
        return;
    }
    ...
    /* macOS accent picker suppresses TEXT_INPUT during key-repeat; fall back
     * to synth_repeat_char so held keys still emit output. */
    if (0 == len && event->key.repeat && !(mod & SDL_KMOD_CTRL) &&
        !(mod & SDL_KMOD_GUI)) {
