...
/* Byte index of the codepoint whose last byte is buf[pos-1].
 * Skips continuation bytes (0x80..0xBF) walking left. */
...
/* Decodes one codepoint from *s, advancing *s. Returns U+FFFD on error. */
static inline uint32_t utf8_decode(const unsigned char **s) {
    ...
    if (b0 < 0x80u) {
        ...
        if ((b1 & 0xC0u) == 0x80u) {
            (*s)++;
            cp = ((uint32_t)(b0 & 0x1Fu) << 6) | (b1 & 0x3Fu);
        }
        ...
        if ((b1 & 0xC0u) == 0x80u && (b2 & 0xC0u) == 0x80u) {
            ...
            cp = ((uint32_t)(b0 & 0x0Fu) << 12) |
                 ((uint32_t)(b1 & 0x3Fu) << 6) | (b2 & 0x3Fu);
            if (cp < 0x0800u || (cp >= 0xD800u && cp <= 0xDFFFu))
                cp = 0xFFFDu;
        }
