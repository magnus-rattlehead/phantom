...
#define AC_TOCK(label, t0) AC_LOG("%-20s %6.1f ms", (label), ac_ms_since(t0))
#else
#define AC_TICK(t0)
...
/* Appends to buf[pos..cap); truncates silently; buf always NUL-terminated. */
...
/* Returns 1 if the query likely involves paths; 0 to skip FS context noise. */
static int query_needs_fs(const char *q) {
    ...
    static const char *const file_cmds[] = {
        "vim",   "vi",    "nvim",   "nano",    "emacs", "cat",  "less",
        "more",  "head",  "tail",   "cp",      "mv",    "rm",   "ln",
        "chmod", "chown", "open",   "code",    "subl",  "gcc",  "clang",
        "cc",    "g++",   "python", "python3", "ruby",  "node", "go",
        "diff",  "patch", "wc",     "grep",    "awk",   "sed",  "source",
        ".",     "sh",    "bash",   "zsh",     NULL};
    char word[AC_WORD_BUF];
    extract_first_word(q, word, sizeof word);
    for (int k = 0; file_cmds[k]; k++)
        if (0 == strcmp(word, file_cmds[k]))
            return 1;
    return 0;
}

static const char *const suggestion_patterns[] = {
    "did you mean", "try:", "hint:", "note:", "run '", NULL};

static int line_has_suggestion(const char *line, size_t len) {
    /* Null-terminate a copy so strcasestr can work on the bounded line. */
    char tmp[512];
    if (len >= sizeof tmp)
        len = sizeof tmp - 1;
    memcpy(tmp, line, len);
    tmp[len] = '\0';
    for (int i = 0; suggestion_patterns[i]; i++) {
        if (NULL != strcasestr(tmp, suggestion_patterns[i]))
            return 1;
    }
    return 0;
}

static int extract_suggestion(const char *text, char *out, size_t cap) {
    const char *p = text;
    while (*p) {
        const char *eol = strchr(p, '\n');
        size_t len = eol ? (size_t)(eol - p) : strlen(p);
        if (line_has_suggestion(p, len)) {
            const char *s = p;
            while (len > 0 && (' ' == *s || '\t' == *s)) {
                s++;
                len--;
 ...
 */
size_t build_fim_prefix(History *history, Terminal *term, TerminalState *ts,
                        CCG *ccg, const char *last_cmd, const char *query,
                        char *out, size_t out_cap) {
    if (NULL == out || 0 == out_cap)
        return 0;
    ...
    if (have_cmd || exit_code > 0) {
        if (have_cmd && exit_code > 0)
            pos += apnd(out, out_cap, pos, "[LAST_CMD]: %s (exit %d)\n",
                        last_cmd, exit_code);
        else if (have_cmd)
            pos += apnd(out, out_cap, pos, "[LAST_CMD]: %s\n", last_cmd);
        else
            pos +=
                apnd(out, out_cap, pos, "[LAST_CMD]: (exit %d)\n", exit_code);
    }
    if (exit_code > 0 && NULL != ts) {
        ...
        if (raw_len > 0) {
            ...
            if ('\0' != stripped[0]) {
                ...
                if (extract_suggestion(stripped, suggestion, sizeof suggestion))
                    pos += apnd(out, out_cap, pos, "[SUGGESTION]: %s\n",
                                suggestion);
    ...
    if (NULL != ccg) {
        ...
        if (nk > 0) {
            ...
            for (int i = 0; i < nk; i++)
                pos += apnd(out, out_cap, pos, "%s%s", (0 == i) ? " " : ", ",
                            top[i]);
            pos += apnd(out, out_cap, pos, "\n");
        }
    }

    int need_fs = query_needs_fs(query);
    ...
    /* [DIR] placed last: recency bias makes it highest-weight for the LLM. */
    if (NULL != ts && need_fs) {
        terminal_state_lock(ts);
...
/* ALL non-flag args are paths; bare names (no . or /) are still checked. */
static const char *const g_all_path_cmds[] = {
    "cat",  "less", "more", "head", "tail", "diff", "wc",
    "file", "du",   "ls",   "cd",   "open", NULL};

/* Only path-shaped tokens (containing . or /) are checked; first arg may be
 * a pattern or script name. */
static const char *const g_some_path_cmds[] = {
    "grep", "egrep", "fgrep", "rg", "ag",  "python", "python3", "node",
    "ruby", "perl",  "bash",  "sh", "zsh", "fish",   "source",  NULL};
...
static int path_cmd_mode(const char *cmd) {
    for (int i = 0; g_all_path_cmds[i]; i++)
        if (0 == strcmp(cmd, g_all_path_cmds[i]))
            return 1;
    for (int i = 0; g_some_path_cmds[i]; i++)
        if (0 == strcmp(cmd, g_some_path_cmds[i]))
            return 2;
    return 0;
}

/* Returns 0 if suggestion references a relative path absent under cwd.
 * Skips stat entirely for commands not in g_all/some_path_cmds. */
static int suggestion_valid(const char *query, const char *suggestion,
                            const char *cwd) {
    if (!suggestion || !cwd || '\0' == cwd[0])
        return 1;
...
/* Returns 1 if a filesystem completion was found and stored in ac->suggestion.
 * Sets *skip_llm_out to 1 when LLM should be bypassed.
 * require_slash: when 1, only proceed if the extracted partial contains '/'. */
static int try_fs_complete(Autocomplete *ac, const char *query,
                           int require_slash, int *skip_llm_out) {
    char partial[PATH_MAX];
    if (1 != extract_path_arg(query, partial, sizeof partial))
        return 0;
    if (require_slash && NULL == strchr(partial, '/'))
        return 0;
    *skip_llm_out = 1;
    char cwd_snap[PATH_MAX];
    terminal_state_lock(ac->ts);
    snprintf(cwd_snap, sizeof cwd_snap, "%s", terminal_state_cwd(ac->ts));
    terminal_state_unlock(ac->ts);
    if ('\0' == cwd_snap[0])
        return 0;
    char suffix[NAME_MAX + 2];
    int n = fs_complete_path(cwd_snap, partial, suffix, sizeof suffix);
    if (n < 0)
        return 0;
    AC_LOG("fs complete hit: \"%s\"", suffix);
    int found = 0;
...
/* Strips a leading markdown code fence from instruct output in-place.
 * Frees s and returns a new heap string, or NULL if body is empty. */
...
/* Worker pipeline: Phase 1 (CCG/HTrie/Schema, under lock) -> Phase 1.5
 * (make/npm/FS, no lock) -> instant wakeup on hit -> Phase 2 (LLM). */
static void *worker(void *arg) {
    ...
    while (ac->running) {
        ...
        if (ac->prepin && !ac->pending) {
            ...
            if (ac->last_cmd)
                snprintf(prepin_snap, sizeof prepin_snap, "%s", ac->last_cmd);
#if defined(__APPLE__)
            ...
            if (ac->ts)
                terminal_state_probe_screen(ac->ts, ac->term);
            ...
            if (ac->llm_base) {
                ...
                if (pbuf) {
                    build_fim_prefix(ac->history, ac->term, ac->ts, ac->ccg,
                                     prepin_snap, "", pbuf, pcap);
        ...
        if (!instant_ready && NULL != ac->ccg) {
            const char *cwd =
                (NULL != ac->ts) ? terminal_state_cwd(ac->ts) : "";
            int exit_code =
                (NULL != ac->term) ? terminal_exit_code(ac->term) : -1;
            uint64_t h = ccg_hash_state(cwd, ac->last_cmd ? ac->last_cmd : "",
                                        exit_code);
            uint32_t freq = 0;
            ...
            if (NULL != instant) {
                ...
                if (typed_len == 0 ||
                    (strncmp(instant, query, typed_len) == 0 &&
                     '\0' != instant[typed_len])) {
                    AC_LOG("CCG instant hit: \"%s\" (freq=%u)", instant, freq);
                    free(ac->suggestion);
        ...
        if (!instant_ready && NULL != ac->htrie &&
            ...
            const char *best = htrie_best(ac->htrie, query, HTRIE_MIN_FREQ);
            if (NULL != best) {
                size_t typed_len = strlen(query);
        ...
        if (ac->last_cmd)
            snprintf(last_cmd_snap, sizeof last_cmd_snap, "%s", ac->last_cmd);
        ...
        if (!instant_ready && NULL != ac->ts) {
            ...
            while (' ' == *p || '\t' == *p)
                p++;
            int is_make =
                (0 == strncmp(p, "make ", 5) || 0 == strncmp(p, "make\t", 5));
            ...
            if (is_make || is_npm) {
                ...
                terminal_state_lock(ac->ts);
                const char *pool = is_make ? terminal_state_make_targets(ac->ts)
                ...
                if (NULL != pool && '\0' != pool[0]) {
                    ...
                    if (1 == n_matches) {
                        ...
                        size_t ml = me ? (size_t)(me - match) : strlen(match);
                        if (ml > typed_len) {
        ...
        if (!instant_ready && NULL != ac->ts) {
            ...
            /* is_path_cmd: all args are paths, no slash guard required.
             * else: only complete if the partial already looks like a path. */
            int require_slash = is_path_cmd ? 0 : 1;
            if (try_fs_complete(ac, query, require_slash, &skip_llm))
                instant_ready = 1;
        }
        ...
        if (QTYPE_COMPLETION == qtype) {
            ...
            }
            {
                AC_TICK(_t0);
                build_fim_prefix(ac->history, ac->term, ac->ts, ac->ccg,
                                 last_cmd_snap, query, prefix_buf, prefix_cap);
                AC_TOCK("build_prefix", _t0);
            }

            int prefix_changed = !ac->last_prefix_base ||
                                 0 != strcmp(ac->last_prefix_base, prefix_buf);
            if (prefix_changed) {
            ...
            /* ChatML prefix for instruct model. */
            char nl_prefix[PATH_MAX + 320];
            size_t p = 0;
            p += (size_t)snprintf(
                nl_prefix + p, sizeof nl_prefix - p,
                "<|im_start|>system\n"
            ...
            int prefix_changed =
                !ac->last_prefix_instruct ||
                0 != strcmp(ac->last_prefix_instruct, nl_prefix);
            if (prefix_changed) {
        ...
        if (QTYPE_NL == qtype) {
            /* ChatML: user text + turn boundaries + assistant start. */
            snprintf(sfx_buf, sfx_cap,
                     "%s\n<|im_end|>\n<|im_start|>assistant\n", query);
        } else {
            /* FIM template for base model. */
            snprintf(sfx_buf, sfx_cap, "%s<|fim_suffix|>\n<|fim_middle|>",
                     query);
        }

        /* USER_INITIATED: P-cores, below render/PTY main thread priority. */
#if defined(__APPLE__)
        pthread_set_qos_class_self_np(QOS_CLASS_USER_INITIATED, 0);
#endif
        {
        ...
        /* Generate from combined seq-0+seq-1 KV state; NL queries continue
         * in blocks to support multi-line script output. */
        int allow_nl = (QTYPE_NL == qtype);
        char *result = NULL;
        {
            AC_TICK(_t0);
            result = llm_complete_from_seq(llm, max_tok, allow_nl, &ac->cancel);
            if (allow_nl && result) {
                size_t total_bytes = strlen(result);
                size_t cap = total_bytes + 1;
                while (!ac->cancel) {
                    char *more =
                        llm_continue(llm, max_tok, allow_nl, &ac->cancel);
                    if (!more)
                        break;
        ...
        /* Validate paths only for single-line completions; NL/multiline output
         * may reference files not yet created, so skip validation there. */
        if (result && ac->ts && QTYPE_COMPLETION == qtype &&
            ...
            if (cwd_ptr)
                memcpy(cwd_snap, cwd_ptr, PATH_MAX);
            terminal_state_unlock(ac->ts);
...
static int extract_path_arg(const char *query, char *out, size_t cap) {
    ...
    while (tok > query && ' ' != tok[-1])
        tok--;

    if (tok == query) {
        /* First token: guard against completing command names as paths. */
        if ('/' != tok[0] && '~' != tok[0] && NULL == strchr(tok, '/'))
            return -1;
    }
    if ('-' == tok[0])
        return -1;
...
void autocomplete_record_command(Autocomplete *ac, const char *cmd,
                                 int exit_code) {
    if (NULL == ac || NULL == ac->ccg || NULL == cmd)
        return;

    pthread_mutex_lock(&ac->lock);
    const char *cwd = (NULL != ac->ts) ? terminal_state_cwd(ac->ts) : "";
    uint64_t h =
        ccg_hash_state(cwd, ac->last_cmd ? ac->last_cmd : "", exit_code);
...
void autocomplete_request_env_probe(Autocomplete *ac) {
    if (NULL != ac)
        fsprobe_request_env_probe(ac->fsprobe);
}
