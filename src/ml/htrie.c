...
void htrie_destroy(HTrie *t)
{
    ...
    for (size_t i = 0; i < t->n; i++) free(t->entries[i].cmd);
    free(t->entries);
...
void htrie_insert(HTrie *t, const char *cmd)
{
    ...
    if (t->n >= t->cap) {
        ...
        if (!ne) { pthread_mutex_unlock(&t->lock); return; }
        t->entries = ne;
        t->cap     = ncap;
    }
    char *copy = strdup(cmd);
    if (!copy) { pthread_mutex_unlock(&t->lock); return; }
...
{
    if (!t || !prefix || '\0' == prefix[0]) return NULL;
    ...
    for (size_t i = 0; i < t->n; i++) {
        /* Sorted descending; once count drops below threshold, no more matches. */
        if (t->entries[i].count < min_count) break;
