...
#define LLM_TOK_BUF    2048  /* max tokens for prompt tokenisation */
#define LLM_PIECE_BUF    16  /* bytes per token piece */
...
struct LLM {
    ...
    volatile int               ready;  /* true once model+ctx are initialised */
    ...
    llama_pos                  gen_pos;       /* next position to decode */
    llama_token                gen_last_tok;  /* last sampled token */
    int                        gen_done;      /* EOS or error hit */
};
...
static void *load_worker(void *arg)
{
    ...
    LLM_LOG("loading model: %s (gpu_layers=%d)",
            l->model_path_copy, LLM_GPU_LAYERS);
    ...
    l->sampler = llama_sampler_chain_init(
    ...
    LLM_LOG("warming up Metal pipelines...");
    {
        ...
        llama_token *hist_toks = malloc(192 * sizeof(llama_token));
        if (hist_toks) {
            for (int i = 0; i < 192; i++) hist_toks[i] = tok;
            struct llama_batch bhist = llama_batch_get_one(hist_toks, 192);
            (void)llama_decode(l->ctx, bhist);
            free(hist_toks);
        }

        llama_pos suf_pos = 192;
        for (int suf_n = 3; suf_n <= 8; suf_n++) {
...
{
    if (!model_path || '\0' == model_path[0]) return NULL;

    LLM *l = calloc(1, sizeof *l);
    if (!l) return NULL;

    l->model_path_copy = strdup(model_path);
    if (!l->model_path_copy) { free(l); return NULL; }
...
{
    if (!l) return;
    if (l->load_thread) pthread_join(l->load_thread, NULL);
    if (l->sampler) llama_sampler_free(l->sampler);
                          ...
                          llama_token **out_toks)
{
    ...
    int n = llama_tokenize(vocab, text, (int32_t)strlen(text),
                           toks, LLM_TOK_BUF, /*add_special=*/false,
                           /*parse_special=*/true);
...
{
    if (!l || !l->ready || !history_prefix) return -1;
    ...
    /* Probe the batch_init path so decode_suffix starts with warm GPU state. */
    {
        struct llama_batch w = llama_batch_init(4, 0, 1);
        w.n_tokens = 4;
        for (int i = 0; i < 4; i++) {
            w.token[i]     = 0;
...
int llm_decode_suffix(LLM *l, const char *suffix_text,
                      volatile int *cancel)
{
    if (!l || !l->ready || !suffix_text) return -1;
    if (cancel && *cancel) return -1;

    llama_memory_t mem = llama_get_memory(l->ctx);
    llama_memory_seq_rm(mem, 0, (llama_pos)l->history_n_tokens, -1);

    if ('\0' == *suffix_text) { l->suffix_n_tokens = 0; return 0; }
...
/* Shared generation loop.  Starts sampling from current KV state at
 * start_pos, writes up to max_tokens into a new heap string.
                      ...
                      int allow_nl, volatile int *cancel)
{
    ...
    for (size_t i = 0; i < max_tokens; i++) {
        if (cancel && *cancel) { l->gen_done = 1; break; }

        llama_token tok = llama_sampler_sample(l->sampler, l->ctx, -1);
        llama_sampler_accept(l->sampler, tok);

        if (llama_vocab_is_eog(l->vocab, tok)) { l->gen_done = 1; break; }
        if (!allow_nl && tok == nl_tok)         { l->gen_done = 1; break; }

        int n_piece = llama_token_to_piece(l->vocab, tok,
                                           piece, (int32_t)sizeof piece,
                                           /*lstrip=*/0, /*special=*/false);
        if (n_piece <= 0) { l->gen_done = 1; break; }

        if (result_len + (size_t)n_piece + 1 > result_cap) {
            result_cap *= 2;
            char *nb = realloc(result, result_cap);
            if (!nb) { l->gen_done = 1; break; }
            result = nb;
        }
        ...
        next.token[0] = tok;
        next.pos[0]   = next_pos++;
        if (0 != llama_decode(l->ctx, next)) { l->gen_done = 1; break; }
    }
    llama_batch_free(next);

    if (0 == result_len) { free(result); return NULL; }
    return result;
}
                             ...
                             volatile int *cancel)
{
    ...
    llama_pos start =
        (llama_pos)(l->history_n_tokens + l->suffix_n_tokens);

    char *result = gen_loop(l, start, max_tokens, allow_nl, cancel);
    LLM_LOG("complete_from_seq: %s -> \"%s\"",
...
char *llm_continue(LLM *l, size_t max_tokens, int allow_nl,
                   volatile int *cancel)
{
    if (!l || !l->ready || l->gen_done) return NULL;

    char *result = gen_loop(l, l->gen_pos, max_tokens, allow_nl, cancel);
    LLM_LOG("llm_continue: %s -> \"%s\"",
            result ? "ok" : "NULL", result ? result : "");
    return result;
}
