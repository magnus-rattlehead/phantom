 ...
 * partial_arg is the last non-flag token from the query (e.g. "fi", "src/ml/a").
 * If exactly one directory entry in the implied parent dir matches the prefix:
 *   writes the name-remainder (plus "/" for directories) into out[0..cap),
 *   NUL-terminates, and returns byte count (0 for exact match).
...
void           terminal_state_probe_screen(TerminalState *ts,
                                           Terminal *term);
