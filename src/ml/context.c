...
/* Encodes Unicode codepoint cp as UTF-8 into out (≥4 bytes).
 * Returns number of bytes written (1–4), or 0 for invalid cp. */
...
static int fs_ext_priority(const char *name, int is_dir)
{
    ...
    if (!dot || '\0' == dot[1]) return 1; /* no/trailing dot: executable etc. */

    /* Omit build artifacts  -  pure noise for the LLM. */
    static const char *const lo[] = {
        ".o", ".a", ".so", ".dylib", ".dll", ".exe",
        ".bin", ".lock", ".class", ".pyc", ".pyo",
        ".wasm", ".map", ".d", NULL
    };
    for (int i = 0; lo[i]; i++) {
        if (0 == strcmp(dot, lo[i])) return -1;
    }

    /* High-value: source files the LLM can reason about. */
    static const char *const hi[] = {
        ".c", ".h", ".cpp", ".cc", ".cxx", ".hpp", ".hh",
        ".go", ".py", ".js", ".ts", ".jsx", ".tsx",
        ".rs", ".java", ".rb", ".swift", ".kt", ".cs",
        ".zig", ".nim", NULL
    };
    for (int i = 0; hi[i]; i++) {
        if (0 == strcmp(dot, hi[i])) return 4;
    }

    /* Medium-value: config, docs, scripts. */
    ...
    for (int i = 0; md[i]; i++) {
        if (0 == strcmp(dot, md[i])) return 3;
    }

    return 0; /* unknown extension */
}

/* One collected directory entry (stack-allocated array, so name is bounded). */
...
/* Flat ls-style listing of dir entries into buf[*pos..cap). */
static void fs_list_cwd(const char *dir, char *buf, size_t cap, size_t *pos)
{
    ...
    /* Emit dirs, then files; space-separated. */
    for (int pass = 0; pass < 2; pass++) {
        for (int i = 0; i < nentries; i++) {
            ...
            if (*pos + need + 1 >= cap) break;
            if (*pos > 0 && ' ' != buf[*pos - 1])
                buf[(*pos)++] = ' ';
            memcpy(buf + *pos, entries[i].name, nlen);
            *pos += nlen;
            if (entries[i].is_dir) buf[(*pos)++] = '/';
        }
    }
...
static void fs_scan(const char *root, int max_depth,
                    char *buf, size_t cap, size_t *pos)
{
    static const char *const skip_dirs[] = {
        "build", "target", "node_modules", "vendor", ".git", NULL
    };
    ...
    while (head < tail && *pos + 4 < cap) {
        ...
        /* Collect all entries for this directory before writing anything. */
        FsEnt entries[FS_MAX_ENTRIES];
        int   nentries = collect_dir_entries(d, entries, FS_MAX_ENTRIES);
        closedir(d);
        qsort(entries, (size_t)nentries, sizeof *entries, fs_ent_cmp);

        /* rel is the path suffix after root, e.g. "/src/ml".
         * Empty suffix means we're at the root itself -> display as ".". */
        ...
        /* Emit directories first, then a " | " separator, then files.
         * Trailing "/" on dir names is the secondary marker; the two groups
         * give the LLM an unambiguous visual split between navigable dirs
         * and referenceable files. */
        int emitted_dirs  = 0;
        int emitted_files = 0;
        ...
        for (int i = 0; i < nentries && *pos + 2 < cap; i++) {
            ...
            *pos += nlen;
            ...
            if (job->depth < max_depth && tail < FS_QUEUE_CAP) {
                ...
                if (!skip) {
                    snprintf(queue[tail].path, PATH_MAX,
                             "%s/%s", job->path, entries[i].name);
        ...
        for (int i = 0; i < nentries && *pos + 2 < cap; i++) {
            ...
            buf[(*pos)++] = ' ';
            size_t nlen = strlen(entries[i].name);
            if (*pos + nlen + 1 >= cap) break;
            memcpy(buf + *pos, entries[i].name, nlen);
            *pos += nlen;
            emitted_files++;
        }

        /* Suppress empty lines (all entries filtered). */
...
static char *parse_make_targets(const char *cwd)
{
    static const char *const names[] = {
        "Makefile", "makefile", "GNUmakefile", NULL
    };
    FILE *f = NULL;
    ...
    if (NULL == buf) { fclose(f); return NULL; }
    size_t pos = 0;
    ...
    while (NULL != fgets(line, (int)sizeof line, f)) {
        char *c = line;
        if ('\t' == *c || '.' == *c || '%' == *c || '#' == *c) continue;
        if (!isalnum((unsigned char)*c) && '_' != *c) continue;
        ...
        if (0 == name_len || pos + name_len + 2 > MAKE_TARGETS_CAP) continue;
...
static char *parse_pkg_scripts(const char *cwd)
{
    ...
    while (NULL != fgets(line, (int)sizeof line, f)) {
        ...
        if (0 == name_len || name_len > 64) continue;

        char *after = q2 + 1;
        while (' ' == *after || '\t' == *after) after++;
        if (':' != *after) continue;

        if (pos + name_len + 2 > MAKE_TARGETS_CAP) break;
...
int fs_complete_path(const char *cwd, const char *partial_arg,
                     char *out, size_t cap)
{
    ...
    if ('/' == partial_arg[0]) {
        if (NULL != last_slash) {
            ...
            if (0 == dir_len) {
                ...
                snprintf(scan_path, sizeof scan_path, "%.*s",
                         (int)dir_len, partial_arg);
            }
            file_prefix = last_slash + 1;
        } else {
            snprintf(scan_path, sizeof scan_path, "/");
            file_prefix = partial_arg + 1;
        }
        CTX_LOG("fs_complete: absolute arg \"%s\" -> scan \"%s\"",
                partial_arg, scan_path);
    } else if (NULL != last_slash) {
        size_t dir_len = (size_t)(last_slash - partial_arg);
        char raw[PATH_MAX];
        snprintf(raw, sizeof raw, "%s/%.*s", cwd, (int)dir_len, partial_arg);
        if (NULL == realpath(raw, scan_path)) return -1;
        file_prefix = last_slash + 1;
    ...
    /* Scan all matches; track longest common prefix (LCP) across names.
     * Single match -> full suffix + dir slash.
     * Multiple matches -> LCP suffix (no slash) if it extends the prefix. */
    char   lcp[NAME_MAX + 1];
    size_t lcp_len      = 0;
    ...
    while (NULL != (ent = readdir(d))) {
        if ('.' == ent->d_name[0]) continue;
        if (0 != strncmp(ent->d_name, file_prefix, prefix_len)) continue;
...
{
    if (NULL == ts || NULL == cwd || '\0' == cwd[0]) return;
    pthread_mutex_lock(&ts->lock);
...
void terminal_state_probe_env(TerminalState *ts)
{
    ...
    if (shell_pid > 0) {
        ...
        int n = proc_pidinfo((int)shell_pid, PROC_PIDVNODEPATHINFO, 0,
                             &vnpi, (int)sizeof vnpi);
        if (n >= (int)sizeof(struct proc_vnodepathinfo)) {
            snprintf(new_cwd, sizeof new_cwd, "%s",
                     vnpi.pvi_cdir.vip_path);
            CTX_LOG("probe_env: proc_pidinfo -> \"%s\"", new_cwd);
        } else {
            CTX_LOG("probe_env: proc_pidinfo failed (pid=%d n=%d)", (int)shell_pid, n);
        }
    } else {
    ...
    CTX_LOG("probe_env: new=\"%s\" cached=\"%s\" changed=%d",
            new_cwd, ts->cached_cwd, cwd_changed);
    if (cwd_changed) {
    ...
    if ('\0' != new_cwd[0]) {
        ...
        if (NULL != new_fs) {
            fs_scan(new_cwd, CONTEXT_FS_DEPTH,
                    new_fs, CONTEXT_FS_BYTES, &new_len);
    ...
    if ('\0' != new_cwd[0]) {
        ...
        if (NULL != new_listing) {
            fs_list_cwd(new_cwd, new_listing,
                        CONTEXT_CWD_LISTING_BYTES, &new_listing_len);
...
void terminal_state_probe_screen(TerminalState *ts, Terminal *term)
{
    ...
    Cell *cells = malloc((size_t)(cols * rows) * sizeof *cells);
    if (NULL == cells) return;

    int cur_col, cur_row;
    terminal_get_state(term, cells, &cur_col, &cur_row);

    char *buf = malloc(CONTEXT_SB_BYTES);
    if (NULL == buf) { free(cells); return; }
    ...
    for (int r = 0; r < rows; r++) {
        int row_end = cols - 1;
        while (row_end > 0 && ' ' == cells[r * cols + row_end].ch)
            row_end--;
        for (int cc = 0; cc <= row_end; cc++) {
            uint32_t cp = cells[r * cols + cc].ch;
            size_t nb = encode_utf8(cp, piece);
            if (0 == nb || pos + nb >= (size_t)CONTEXT_SB_BYTES - 1) break;
            memcpy(buf + pos, piece, nb);
            pos += nb;
        }
        if (pos < (size_t)CONTEXT_SB_BYTES - 1) buf[pos++] = '\n';
    }

    ts->sb_buf = buf;
    ts->sb_len = pos;
    free(cells);
    CTX_LOG("screen snapshot: %zu bytes", pos);
}
...
{
    if (NULL != ts) pthread_mutex_lock(&ts->lock);
}

void terminal_state_unlock(TerminalState *ts)
{
    if (NULL != ts) pthread_mutex_unlock(&ts->lock);
}
...
size_t terminal_state_recent_bytes(const TerminalState *ts,
                                   char *out_buf,
                                   size_t max_bytes)
{
    if (NULL == ts || NULL == ts->sb_buf || 0 == max_bytes) return 0;
