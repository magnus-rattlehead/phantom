...
/* Pins history_prefix in KV seq 0 and copies to seq 1. No-op when text is
 * unchanged. Returns token count on success, -1 on error. */
int llm_pin_history(LLM *l, const char *history_prefix);

/* Rewinds seq 1 to the end of the pinned history, then decodes suffix_text.
