...
typedef struct {
    ...
    uint8_t _pad;
    uint16_t url_idx; /* 0 = no URL; 1-based index into Terminal url_pool */
} Cell;
...
/* Allocates a terminal of cols x rows cells with default colors. */
...
/* Selection (main-thread only  -  no mutex, call only from event/render
 * thread). */
...
/* Returns malloc'd UTF-8 text for the current selection; NULL if none. Caller
 * frees. */
char *terminal_get_selected_text(Terminal *t);

/* Copies the full cell grid and cursor position; thread-safe (locks
 * internally). */
...
/* Discard all scrollback rows and snap to bottom. Thread-safe. */
...
/* Returns a malloc'd copy of the URL at cell (col, row) in the current
 * rendered view, or NULL if no URL.  Caller frees.  Thread-safe. */
char *terminal_get_url_at(Terminal *t, int col, int row);

/* Fills *col and *row with current cursor position. */
