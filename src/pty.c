...
#define PTY_READ_BUF_LEN 65536 /* bytes per read() call from the master fd */
...
static void pty_reap_event(void *ctx) {
    PtyReapCtx *rc = (PtyReapCtx *)ctx;
    int st = 0;
    if (waitpid(rc->pty->child_pid, &st, 0) > 0 && WIFEXITED(st))
        rc->pty->last_child_exit = WEXITSTATUS(st);
    if (rc->pty->on_exit)
        rc->pty->on_exit(rc->pty->callback_arg);
    dispatch_source_cancel(rc->src);
}

static void pty_reap_cancel(void *ctx) {
    PtyReapCtx *rc = (PtyReapCtx *)ctx;
    dispatch_release(rc->src);
    free(rc);
}

static void pty_reap_child_async(Pty *pty) {
    PtyReapCtx *ctx = malloc(sizeof *ctx);
    if (!ctx)
        return;
    dispatch_source_t src = dispatch_source_create(
        DISPATCH_SOURCE_TYPE_PROC, (uintptr_t)pty->child_pid,
        DISPATCH_PROC_EXIT,
        dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0));
    ctx->pty = pty;
    ctx->src = src;
    dispatch_set_context(src, ctx);
    dispatch_source_set_event_handler_f(src, pty_reap_event);
    dispatch_source_set_cancel_handler_f(src, pty_reap_cancel);
    dispatch_resume(src);
}
...
int pty_open(Pty *pty, int cols, int rows) {
    ...
    /* self-pipe for waking the reader thread on shutdown */
    ...
    if (0 == pid) {
        ...
        if (-1 == ioctl(slave_fd, TIOCSCTTY, 0))
            _exit(1);
        if (-1 == dup2(slave_fd, STDIN_FILENO))
            _exit(1);
        if (-1 == dup2(slave_fd, STDOUT_FILENO))
            _exit(1);
        if (-1 == dup2(slave_fd, STDERR_FILENO))
            _exit(1);
        close(slave_fd);
        close(master_fd);
        close(pty->wakeup_pipe[0]);
    ...
    close(slave_fd);
    /* Non-blocking so the drain loop can read until EAGAIN. */
...
static void *pty_reader_thread(void *arg) {
    ...
    /* set on first byte received; used to erase the "Loading..." overlay */
    int first_read = 1;
    ...
    while (!pty->stop_flag) {
        ...
        /* select(2) requires highest fd + 1 */
        int nfds =
            (pty->master_fd > pty->wakeup_pipe[0] ? pty->master_fd
                                                  : pty->wakeup_pipe[0]) +
            1;

        if (select(nfds, &rfds, NULL, NULL, NULL) < 0)
            break;
        /* wakeup_pipe[0] readable means pty_close() signalled shutdown */
        if (FD_ISSET(pty->wakeup_pipe[0], &rfds))
            break;

        if (FD_ISSET(pty->master_fd, &rfds)) {
            /* drain until EAGAIN or child exit */
            while (!pty->stop_flag) {
                ssize_t n = read(pty->master_fd, buf, sizeof buf);
                if (n > 0) {
                    if (first_read) {
                        first_read = 0;
                        /* CR + erase-to-EOL clears the loading overlay */
                        terminal_feed(pty->terminal, "\r\x1b[K", 4);
                    }
                    terminal_feed(pty->terminal, buf, (size_t)n);
                    if (pty->on_data)
                        pty->on_data(pty->callback_arg);
                } else if (n == 0 ||
                           (n < 0 && errno != EAGAIN && errno != EWOULDBLOCK)) {
#if defined(__APPLE__)
                    pty_reap_child_async(pty);
#else
                    int status = 0;
...
void pty_close(Pty *pty) {
    ...
    if (pty->master_fd >= 0)
        close(pty->master_fd); /* closing master sends SIGHUP to shell */
    pty->master_fd = -1;
    close(pty->wakeup_pipe[0]);
    close(pty->wakeup_pipe[1]);
    ...
    if (pty->child_pid > 0) {
#if !defined(__APPLE__)
        /* On macOS the dispatch source reaps the child asynchronously;
         * on other platforms use the blocking SIGTERM/SIGKILL fallback. */
